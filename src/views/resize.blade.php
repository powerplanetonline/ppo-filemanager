    ...
    <div class="col-md-4 fill">
      ...
      <table class="table table-compact table-striped">
        ...
        <tbody>
          @if ($scaled)
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-ratio') }}</td>
            <td>{{ number_format($ratio, 2) }}</td>
          </tr>
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-scaled') }}</td>
            <td>
              {{ Lang::get('ppo-filemanager::lfm.resize-true') }}
            </td>
          </tr>
          @endif
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-old-height') }}</td>
            <td>{{ $original_height }}px</td>
          </tr>
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-old-width') }}</td>
            <td>{{ $original_width }}px</td>
          </tr>
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-new-height') }}</td>
            <td><span id="height_display"></span></td>
          </tr>
          <tr>
            <td>{{ Lang::get('ppo-filemanager::lfm.resize-new-width') }}</td>
            <td><span id="width_display"></span></td>
          </tr>
        </tbody>
      </table>

      <button class="btn btn-primary" onclick="doResize()">{{ Lang::get('ppo-filemanager::lfm.btn-resize') }}</button>
      <button class="btn btn-info" onclick="loadItems()">{{ Lang::get('ppo-filemanager::lfm.btn-cancel') }}</button>
